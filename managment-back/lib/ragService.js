...
const RAG_MEASUREMENT_SCHEMA_RULES = [
  ...
  'If query is not explicitly measurement/comparison, do NOT output JSON; answer in normal prose.',
  'When schema mode is required, return a strict JSON object first with keys:',
  ...
  'CPS comparison is valid only when RPM is explicit and equal for both values; otherwise mark comparable=false and add reason.',
  'RAG-to-experiment linkage priority: exact unit+conditions match, then metric+unit, then metric only (with lower confidence).',
  'Classify evidence source as formulation | experiment_result | qc_data with confidence high|medium|low.',
  'For viscosity/pH, prioritize experiment_result and qc_data evidence; for composition percentages, prioritize formulation evidence.',
  'Keep cross-field consistency: do not combine values from incompatible units/conditions; if conflicts exist, state them in notes.',
...
export class RAGService {
  ...
  async search(query, nResults = 5, filterMetadata = null) {
    ...
    return this.vectorStore.search(semanticQuery, nResults, filterMetadata);
  }

  async generateAnswer(query, nResults = 10, filterMetadata = null, useLlm = true) {
    const semanticQuery = expandQueryWithSemanticTerms(query);
    const rawResults = await this.search(semanticQuery, nResults, filterMetadata);
