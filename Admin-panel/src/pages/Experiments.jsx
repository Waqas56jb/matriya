  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div><h1>Experiments</h1><p>{total} experiments total</p></div>
      ...
      <div className="toolbar">
        <div className="search-bar" style={{ flex: 1, minWidth: 200 }}>
          ...
          <input className="input-field" placeholder="Search by ID or operator…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} />
        </div>
        {[
          { key: 'status',        opts: ['all','PASS','FAIL','PARTIAL','PENDING'], label: 'Status' },
          { key: 'validated',     opts: ['all','true','false'],                   label: 'Validated' },
          { key: 'breakdown_flag',opts: ['all','true','false'],                   label: 'Breakdown' },
        ].map(f => (
      ...
      <div className="section-card" style={{ padding: 0, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div className="empty-state"><div className="icon">🧪</div><p>No experiments found</p></div>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table>
              <thead>
                <tr><th>Experiment ID</th><th>Date</th><th>Operator</th><th>Status</th><th>Decision Shift</th><th>Breakdown</th><th>Validated</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {data.map(exp => (
                  <tr key={exp.id}>
                    ...
                    <td><span className={`badge badge-${exp.decision_shift ? 'warning' : 'muted'}`}>{exp.decision_shift ? 'Yes' : 'No'}</span></td>
                    <td><span className={`badge badge-${exp.breakdown_flag ? 'danger' : 'muted'}`}>{exp.breakdown_flag ? 'Yes' : 'No'}</span></td>
                    <td><span className={`badge badge-${exp.validated ? 'success' : 'muted'}`}>{exp.validated ? 'Yes' : 'No'}</span></td>
                    <td>
                      <button className="btn btn-secondary btn-sm" onClick={() => setEditing({ ...exp })}>✏ Edit</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 500, padding: 20 }}>
          <div className="section-card" style={{ width: '100%', maxWidth: 480 }}>
            <div className="section-title">✏ Edit Experiment — {editing.experiment_id}</div>
            {[
              { key: 'decision_shift', label: 'Decision Shift', type: 'bool' },
              { key: 'breakdown_flag', label: 'Breakdown Flag', type: 'bool' },
              { key: 'validated',      label: 'Validated',      type: 'bool' },
            ].map(f => (
              <div className="input-group" key={f.key} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                <label className="input-label" style={{ marginBottom: 0 }}>{f.label}</label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{editing[f.key] ? 'Yes' : 'No'}</span>
                </label>
              </div>
            ))}
